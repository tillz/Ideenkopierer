...
Twitter.prototype.composeTweet = function(success, failure, tweetText, options){
    var queryString='text='+encodeURIComponent(tweetText);
    if(options.urlAttach!=undefined) {
    queryString+='&url='+encodeURIComponent(options.urlAttach);
    }
    console.log("https://twitter.com/share?"+queryString);
    window.location.href="https://twitter.com/share?"+queryString;
    }
 ...
 */
Twitter.prototype.composeTweet = function(success, failure, tweetText, options){
    window.Tweetoptions = options || {};
    window.Tweetoptions.text = tweetText;
//    cordova.exec(success, failure, "TwitterPlugin", "composeTweet", [options]);
    cordova.exec(function(r) {
if(r==1) {
    cordova.exec(success, failure, "TwitterPlugin", "composeTweet", [window.Tweetoptions]);
} else {
var queryString='text='+encodeURIComponent(window.Tweetoptions.text);
if(window.Tweetoptions.urlAttach!=undefined) {
queryString+='&url='+encodeURIComponent(window.Tweetoptions.urlAttach);
}
console.log("https://twitter.com/share?"+queryString);
window.location.href="https://twitter.com/share?"+queryString;
}
}, null, "TwitterPlugin", "isTwitterSetup", []);
